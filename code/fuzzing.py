...
class Fuzzing(object):
    ...
    def get_policy(self):
        if self.policy_name=="random":
            print("Policy: Random")
            policy = RandomPolicy(self.device,self.app,40,30,40,30,5,5)
        else:
        ...
        while self.now_testcase<self.testcase_count:
            ...
            while self.now_event<self.event_num:
                ...
                #Otherwise, select an event to execute according to the exploration rules
                    ...
                    event = self.policy.choice_event(self.device,self.now_event)
                    if event.view !=None and not event.view.notin(self.keyview_list):
                        continue
                    ...
                    if execute_result == True:
                        ...
                        #Waiting for loading
                        waittime = 0
                        while waittime<10 and (self.device.use(className="android.widget.ProgressBar").exists and self.app.package_name!="com.ss.android.lark" or (self.device.use(text="...").exists and self.app.package_name=="io.github.hidroh.materialistic")):
                            time.sleep(1)
                            waittime=waittime+1
    ...
    def execute_dmf(self,dmfnum,f_dmf):
        ...
        for event in nowdmf.events:
            ...
            #If the execution is successful:
                ...
                time.sleep(1)
                waittime = 0
                while (waittime<5 and self.device.use(resourceId="com.ss.android.lark:id/search_result_loading_txt").exists and self.app.package_name=="com.ss.android.lark" ) or (waittime<10 and self.device.use(className="android.widget.ProgressBar").exists and self.app.package_name!="com.ss.android.lark"):
                    time.sleep(1)
                    waittime=waittime+1
                #Update screen
                self.now_event=self.now_event+1
